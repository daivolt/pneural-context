...
Revision ID: 7f2686c2f5df
Revises:
Create Date: 2026-07-16 13:03:21.440131
...
def upgrade() -> None:
    """Create initial pneural-context schema."""
    op.execute(SCHEMA_SQL)


def downgrade() -> None:
    """Drop all pneural-context tables."""
    op.execute("DROP TABLE IF EXISTS pb_memory_archive")
    op.execute("DROP TABLE IF EXISTS pb_memory")
    op.execute("DROP TABLE IF EXISTS pb_procedural_memory")
    op.execute("DROP TABLE IF EXISTS pb_consolidated_memory")
    op.execute("DROP TABLE IF EXISTS pb_memory_costs")
    op.execute("DROP TABLE IF EXISTS pb_papers")
